...
# Variables for game metrics
game_start_time = pygame.time.get_ticks()  # Record the start time of the game
game_result = None  # Variable to store game result (1 for win, 0 for loss)
...
while running:
    ...
    car_pixel_color = background_image.get_at((int(car_x), int(car_y)))
    ...
    if is_color_in_range(car_pixel_color, RED_LOWER, RED_UPPER):
        ...
        game_result = 0  # Set game result to loss
        running = False
    ...
    if is_color_in_range(car_pixel_color, CYAN_LOWER, CYAN_UPPER):
        ...
        game_result = 1  # Set game result to win
        running = False
...
game_duration = (pygame.time.get_ticks() - game_start_time) / 1000  # Convert to seconds

# Print game metrics
print("Game Duration:", game_duration, "seconds")
print("Game Result:", "Win" if game_result == 1 else "Loss")
